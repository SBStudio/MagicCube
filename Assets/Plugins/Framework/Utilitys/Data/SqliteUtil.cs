...
namespace Framework
{
	public sealed class SqliteUtil
	{
		...
		{
			string command = string.Format(COMMAND_OPEN, name);
		...
		public SqliteDataReader Create(string table, string[] fields, Type[] types)
		{
			...
			command = string.Format(COMMAND_CREATE, table, command);
		...
		{
			string command = string.Format(COMMAND_TEXT, datas[0]);
			for (int i = 1; i < datas.Length; ++i)
			{
				command += COMMAND_SPLIT + string.Format(COMMAND_TEXT, datas[i]);
			}

			command = string.Format(COMMAND_ADD, table, command);
		...
		public SqliteDataReader Add(string table, string[] fields, object[] datas)
		{
			...
			string data = string.Format(COMMAND_TEXT, datas[0]);
			for (int i = 1; i < datas.Length; ++i)
			{
				data += COMMAND_SPLIT + string.Format(COMMAND_TEXT, datas[i]);
			}

			string command = string.Format(COMMAND_ADD_INTO, table, field, data);
		...
		public SqliteDataReader Get(string table, string[] getFields, string[] fields, string[] conditions, object[] datas)
		{
			...
			string command = fields[0] + conditions[0] + string.Format(COMMAND_TEXT, datas[0]);
			for (int i = 1; i < fields.Length; ++i)
			{
				command += COMMAND_AND + fields[i] + conditions[i] + string.Format(COMMAND_TEXT, datas[i]);
			}

			command = string.Format(COMMAND_GET, table, field, command);
		...
		{
			string command = string.Format(COMMAND_GET_ALL, table);
		...
		{
			string get = getData + COMMAND_EQUAL + string.Format(COMMAND_TEXT, getValue);
			string set = setField + COMMAND_EQUAL + string.Format(COMMAND_TEXT, setData);
			string command = string.Format(COMMAND_SET, table, set, get);
		...
		{
			string get = getField + COMMAND_EQUAL + string.Format(COMMAND_TEXT, getData);
			string set = setFields[0] + COMMAND_EQUAL + string.Format(COMMAND_TEXT, setDatas[0]);
			for (int i = 1; i < setDatas.Length; ++i)
			{
				set += COMMAND_SPLIT + setFields[i] + COMMAND_EQUAL + string.Format(COMMAND_TEXT, setDatas[i]);
			}
		...
		{
			string type = fields[0] + COMMAND_EQUAL + string.Format(COMMAND_TEXT, datas[0]);
			for (int i = 1; i < fields.Length; ++i)
			{
				type += COMMAND_OR + fields[i] + COMMAND_EQUAL + string.Format(COMMAND_TEXT, datas[i]);
			}

			string command = string.Format(COMMAND_DELETE, table, type);
		...
		{
			string command = string.Format(COMMAND_CLEAR, table);
